...
begin # Individual Cluster Methods
        ...
        function get_coordinates(bundle::AbstractBundle, cluster)

                ghash = bundle.hashing_fxn(cluster)[1]
                svs = bundle.cluster_info[ghash][4]

                sorted_vertices = sort(unique(vcat(connections(lattice(bundle))[svs]...)))

                bundle.coordinates[sorted_vertices, :]
